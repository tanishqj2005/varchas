...
def ourteam(request):
    ...
    positions = {}
    for position, display in _positions.items():
        if (OurTeam.objects.filter(position=position).exists()):
            positions[position] = display
        return render(request, 'main/our_team.html', {'members': members, 'positions': positions})
