...
class match(Event):
    ...
    game = models.CharField(max_length=50, choices=game_ch)
    match_type = models.CharField(max_length=50, choices=mtype_ch)
    play_status = models.CharField(max_length=50, choices=status_ch, default='2')
    end_comment = models.TextField(null=True)
    winner = models.ForeignKey(TeamRegistration, on_delete=models.CASCADE, related_name="winner", null=True)
    final_score = models.CharField(max_length=50, null=True)
...
class cricket_log(models.Model):
...
class football_logs(models.Model):
...
class volleyball_logs(models.Model):
    match = models.ForeignKey(match, on_delete=models.CASCADE)
